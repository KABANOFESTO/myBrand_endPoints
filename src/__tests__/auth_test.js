...
describe('Testing Auth routes', () => {
  ...
  it("should throw an error if the email value is empty", async () => {
	...
	it('should register a user.', async () => {
		const res = await supertest(baseURL).post('/user/signUp').send((tester));
		expect(res.status).toEqual(201);
		// expect(res.body).toEqual('object');
	});
	it('should login user.', async () => {
        const user = await supertest(baseURL).post('/user/signUp').send(tester);
		const res = await supertest(baseURL).post('/user/login').send({email:user.email,password:user.password});
